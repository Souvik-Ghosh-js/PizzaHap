...
const requestRefund = async (req, res, next) => {
  try {
    ...
    await notifyUser(userId, 'Refund Requested', `Your refund request for order #${order.order_number} has been submitted. Processing within 3-5 business days.`, 'refund', { order_id });
...
const getMyRefunds = async (req, res, next) => {
  try {
    const result = await query(
      `SELECT r.*, o.order_number FROM Refunds r JOIN Orders o ON r.order_id = o.id WHERE r.requested_by = ? ORDER BY r.requested_at DESC`,
      [req.user.id]
    );
...
const getAllRefunds = async (req, res, next) => {
  try {
    ...
    const result = await query(
      `SELECT r.*, o.order_number, o.coins_earned, o.user_id, u.name as user_name, u.email as user_email
...
const processRefund = async (req, res, next) => {
  try {
    ...
    if (action === 'reject') {
      ...
      if (refund.user_id) {
        await notifyUser(refund.user_id, 'Refund Rejected', `Your refund for order #${refund.order_number} was not approved. ${notes || ''}`, 'refund', { order_id: refund.order_id });
      }
    ...
    await query(`UPDATE Orders SET payment_status = 'refunded', status = 'refunded', updated_at = NOW() WHERE id = ?`, [refund.order_id]);

    // Revert coins earned from this order
    if (refund.user_id && refund.coins_earned > 0) {
      await revertCoins(refund.user_id, refund.order_id, refund.coins_earned);
    }

    if (refund.user_id) {
      await notifyUser(refund.user_id, 'Refund Initiated', `Your refund of Rs.${refund.amount} for order #${refund.order_number} has been initiated. It will reflect in 3-5 business days.`, 'refund', { order_id: refund.order_id });
    }
