...
const updateOrderStatus = async (req, res, next) => {
  try {
    ...
    if (status === 'confirmed') {
      deductStockForOrder(req.params.id, req.admin.id).catch(err =>
        logger.error(`Stock deduction failed for order ${req.params.id}: ${err.message}`)
      );
    }
...
const deductStockForOrder = async (orderId, adminId = null) => {
  const items = await query(
    `SELECT oi.product_id, oi.quantity FROM OrderItems oi WHERE oi.order_id = ?`, [orderId]
  );
  for (const item of items) {
    const recipe = await query(
      `SELECT pi.ingredient_id, pi.quantity, i.current_stock, i.low_stock_threshold, i.name, i.location_id
    ...
    for (const r of recipe) {
      const consume = parseFloat((r.quantity * item.quantity).toFixed(3));
      const stockAfter = Math.max(0, parseFloat(r.current_stock) - consume);
      await query(
        `UPDATE Ingredients SET current_stock = ?, updated_at = NOW() WHERE id = ?`,
        [stockAfter, r.ingredient_id]
      );
      await query(
        `INSERT INTO StockLogs (ingredient_id, order_id, change_amount, stock_after, reason, changed_by)
         VALUES (?, ?, ?, ?, 'Order confirmed', ?)`,
        [r.ingredient_id, orderId, -consume, stockAfter, adminId]
      );
      // Low stock alert
      if (stockAfter <= parseFloat(r.low_stock_threshold)) {
        await query(
          `INSERT INTO AdminNotifications (location_id, title, message, type, data)
           VALUES (?, ?, ?, 'system', ?)`,
          [r.location_id || null,
           `Low Stock: ${r.name}`,
           `${r.name} is low — only ${stockAfter} remaining.`,
           JSON.stringify({ ingredient_id: r.ingredient_id, stock: stockAfter })]
        );
      }
    }
  }
...
const getIngredients = async (req, res, next) => {
  try {
    ...
    const params = [];
    if (lid) { where += ' AND (i.location_id = ? OR i.location_id IS NULL)'; params.push(lid); }
    const rows = await query(
      `SELECT i.*, l.name as location_name,
              CASE WHEN i.current_stock <= i.low_stock_threshold THEN 1 ELSE 0 END as is_low_stock
       FROM Ingredients i LEFT JOIN Locations l ON l.id = i.location_id
...
const createIngredient = async (req, res, next) => {
  try {
    const { name, unit = 'pieces', current_stock = 0, low_stock_threshold = 10, location_id } = req.body;
    const resolvedLid = req.admin.location_id || (location_id ? parseInt(location_id) : null);
    const r = await query(
      `INSERT INTO Ingredients (name, unit, current_stock, low_stock_threshold, location_id) VALUES (?,?,?,?,?)`,
      [name.trim(), unit, current_stock, low_stock_threshold, resolvedLid]
    );
    return created(res, { ingredient_id: r.insertId }, 'Ingredient added');
  } catch (err) { next(err); }
};

const updateIngredient = async (req, res, next) => {
  try {
    const { name, unit, current_stock, low_stock_threshold, is_active } = req.body;
    const prev = await query(`SELECT * FROM Ingredients WHERE id = ?`, [req.params.id]);
    if (!prev.length) return notFound(res, 'Ingredient not found');

    await query(
      `UPDATE Ingredients SET
         name                = IFNULL(?,name),
         unit                = IFNULL(?,unit),
         current_stock       = IFNULL(?,current_stock),
         low_stock_threshold = IFNULL(?,low_stock_threshold),
         is_active           = IFNULL(?,is_active),
         updated_at          = NOW()
       WHERE id = ?`,
      [name || null, unit || null,
    ...
    // Log manual restock if stock increased
    if (current_stock != null && parseFloat(current_stock) > parseFloat(prev[0].current_stock)) {
      const diff = parseFloat(current_stock) - parseFloat(prev[0].current_stock);
      await query(
        `INSERT INTO StockLogs (ingredient_id, change_amount, stock_after, reason, changed_by) VALUES (?,?,?,?,?)`,
        [req.params.id, diff, current_stock, 'Manual restock by admin', req.admin.id]
      );
    }
    return success(res, {}, 'Ingredient updated');
  } catch (err) { next(err); }
};

const deleteIngredient = async (req, res, next) => {
  try {
    await query(`UPDATE Ingredients SET is_active = 0 WHERE id = ?`, [req.params.id]);
...
const getLowStockAlerts = async (req, res, next) => {
  try {
    const lid = req.admin.location_id || (req.query.location_id ? parseInt(req.query.location_id) : null);
    let where = 'WHERE i.is_active = 1 AND i.current_stock <= i.low_stock_threshold';
    const params = [];
    if (lid) { where += ' AND (i.location_id = ? OR i.location_id IS NULL)'; params.push(lid); }
    const rows = await query(
      `SELECT i.*, l.name as location_name FROM Ingredients i
       LEFT JOIN Locations l ON l.id = i.location_id ${where} ORDER BY i.current_stock ASC`,
      params
    );
    return success(res, { count: rows.length, items: rows });
  } catch (err) { next(err); }
};
...
const getStockLogs = async (req, res, next) => {
  try {
    const rows = await query(
      `SELECT sl.*, i.name as ingredient_name, i.unit, a.name as changed_by_name
       FROM StockLogs sl
       JOIN Ingredients i ON i.id = sl.ingredient_id
       LEFT JOIN Admins a ON a.id = sl.changed_by
...
const getProductIngredients = async (req, res, next) => {
  try {
    const rows = await query(
      `SELECT pi.*, i.name, i.unit, i.current_stock FROM ProductIngredients pi
...
const setProductIngredients = async (req, res, next) => {
  try {
    const { ingredients } = req.body; // [{ ingredient_id, quantity }]
    if (!Array.isArray(ingredients)) return badRequest(res, 'ingredients must be an array');
    await query(`DELETE FROM ProductIngredients WHERE product_id = ?`, [req.params.id]);
    ...
    return success(res, {}, 'Product ingredients updated');
  } catch (err) { next(err); }
};
